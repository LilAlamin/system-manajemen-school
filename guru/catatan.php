<div class="p-4 sm:ml-64">
    <div class="max-w-6xl mx-auto mt-10">
        ...
        $where_clauses = ["c.c_id_guru = '$teacher_id'"]; 
        ...
        if (!empty($search_query)) {
            $where_clauses[] = "(s.nama_siswa LIKE '%$search_query%' OR k.nama_kelas LIKE '%$search_query%' OR c.catatan LIKE '%$search_query%')";
        }
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6 border border-gray-100">
            <form method="get" action="index.php" class="space-y-4 md:space-y-0 md:flex md:space-x-4 items-end">
                ...
                <div class="w-full md:w-1/4">
                    ...
                    <input type="text" name="search_query" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Cari Siswa, Kelas..." value="<?= isset($search_query) ? htmlspecialchars($search_query) : '' ?>">
                </div>
...
if (isset($_GET['hapus_catatan_id'])) {
    ...
    $teacher_id = $_SESSION['teacher_id'];
    $del = "DELETE FROM `catatan_siswa` WHERE id_catatan = '$id' AND c_id_guru = '$teacher_id'";
    if($conn->query($del)){
        echo "<script>
            Swal.fire({
                title: 'Berhasil!',
